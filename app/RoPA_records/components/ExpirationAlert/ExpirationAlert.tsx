...
function computeRetentionEndDate(retentionStart: string, retentionPeriod: string): Date | null {
...
function daysUntilExpiry(retentionStart: string, retentionPeriod: string): number {
...
function RopaFormReadOnly({
    ...
    const fetchRelated = async () => {
      try {
        ...
        if (resSecurity.ok) {
          ...
          const securityData: any[] = Array.isArray(sJson.data) ? sJson.data : Array.isArray(sJson) ? sJson : [];
      ...
      } catch (err) { console.error('Failed to fetch security data:', err); }
    };
    fetchRelated();
  }, [initialData]);

  const CustomRadio = ({ name, value, checked }: { name: string; value: string; checked: boolean }) => (
  ...
  const inputCls = 'p-2 border border-slate-200 rounded-md text-[14px] text-slate-800 bg-slate-50 w-full';
  const textareaCls = 'p-3 border border-slate-200 rounded-md text-[14px] resize-none text-slate-800 bg-slate-50 w-full';

  const days = daysUntilExpiry(initialData?.retention_start, initialData?.retention_period);
  ...
  return (
    <div className="w-full h-full flex flex-col animate-in fade-in duration-300 overflow-hidden relative">
      ...
      <div className="bg-[#F4F8FB] border border-slate-200 rounded-xl p-8 pt-6 w-full h-full flex flex-col overflow-auto custom-scrollbar">
        <h2 className="text-xl font-bold text-[#1E2A5E] mb-2">รายละเอียดกิจกรรมการประมวลผล</h2>

        {/* Expiry warning banner */}
        <div className={`mb-6 px-5 py-3 rounded-xl flex items-center gap-3 text-sm font-bold ${days <= 0 ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-amber-50 text-amber-700 border border-amber-200'}`}>
        ...
        <div className="flex flex-col gap-8 flex-1">
          ...
          <section className="flex flex-col gap-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[#1E2A5E] font-medium text-[14px]">ระยะเวลาการเก็บรักษา (ปี)</label>
          ...
          <section className="flex flex-col gap-5">
            <h3 className="text-[#1E2A5E] font-bold text-[15px]">คำอธิบายเกี่ยวกับมาตรการรักษาความมั่นคงปลอดภัย</h3>
            <div className="flex flex-col gap-4 pl-4">
              {[
...
export default function ExpirationAlert() {
  ...
  const fetchExpiredRecords = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('http://localhost:3340/ropa-records', { headers: authHeaders() });
      if (res.ok) {
        ...
        const filtered = all.filter(item => {
          ...
          const days = daysUntilExpiry(item.retention_start, item.retention_period);
    ...
    <div className="flex flex-col h-full p-4 animate-in fade-in duration-500 bg-[#F0F9FF]">
      <div className="flex-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar pt-4">
        {isLoading ? (
          ...
          <div className="text-center py-10 text-slate-400 font-bold">ไม่พบรายการที่ใกล้หมดอายุ</div>
        ) : (
          items.map(item => {
            const days = daysUntilExpiry(item.retention_start, item.retention_period);
            ...
            return (
              ...
              >
                <div className="py-6 px-10 flex flex-col gap-1.5">
                  ...
                  <span className={`text-sm font-bold flex items-center gap-1.5 ${isExpired ? 'text-red-500' : 'text-amber-500'}`}>
                    <span>⚠</span>
                    {isExpired
